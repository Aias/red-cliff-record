...
async function processGithubStarsIncremental(integrationRunId: number): Promise<number> {
	...
	while (hasMore) {
			...
			const reachedExisting = data.some((star) => 
					lastKnownDate && new Date(star.starred_at) <= lastKnownDate
			);
			...
			for (const star of data) {
					...
					const existingBookmark = await db.select({ id: bookmarks.id })
							.from(bookmarks)
	...
	if (starsToInsert.length > 0) {
			console.log(`Inserting ${starsToInsert.length} new stars (skipped ${skippedCount} existing stars)`);
			const chunkSize = 100;
...
const main = async () => {
	try {
			await runIntegration(IntegrationType.GITHUB, processGithubStarsIncremental, RunType.INCREMENTAL);
	} catch (err) {
