...
import { browsingHistory, browsingHistoryDaily, Browser, IntegrationType, RunType } from '../../schema/main';
...
async function processArcBrowserHistoryIncremental(integrationRunId: number): Promise<number> {
    ...
    console.log(`Last known visit time: ${lastKnownTime ? new Date(Number(lastKnownTime) / 1000).toISOString() : 'none'}`);

    console.log('Retrieving new history entries...');
    const rawHistory = await dailyVisitsQuery.where(and(
    ...
    if (history.length > 0) {
        ...
        for (let i = 0; i < history.length; i += chunkSize) {
            ...
            await pgDb.insert(browsingHistory)
                .values(chunk);
...
const main = async () => {
    try {
        await runIntegration(IntegrationType.BROWSER_HISTORY, processArcBrowserHistoryIncremental, RunType.INCREMENTAL);
    } catch (err) {
