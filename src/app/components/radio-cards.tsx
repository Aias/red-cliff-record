...
const RadioCardsItem = ({ className, children, ...props }: RadioCardsItemProps) => (
  <RadioGroupPrimitive.Item
    className={cn(
      'relative flex min-h-max cursor-pointer items-center justify-center rounded-md border border-c-border bg-c-paper p-[0.75em] text-left text-c-primary shadow-xs hover:bg-c-mist hover:text-c-display focus:outline-none focus-visible:ring-2 focus-visible:ring-c-ring focus-visible:ring-offset-[-2px] disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-c-ring data-[state=checked]:bg-c-splash',
      className
    )}
    {...props}
  >
