...
export const Route = createFileRoute('/history/$date')({
  loader: ({ params: { date }, context: { queryClient, trpc } }) =>
    queryClient.ensureQueryData(
      trpc.history.getHistoryForDate.queryOptions(date),
    ),
    ...
    <main className="flex basis-full flex-col overflow-hidden p-3">
      <header className="mb-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <nav className="flex gap-2">
          ...
          >
            <Link
              to="/history/$date"
              params={{ date: formatISODate(prevDate) }}
            >
              <ChevronLeftIcon />
              Previous Day
            </Link>
          ...
          >
            <Link
              to="/history/$date"
              params={{ date: formatISODate(nextDate) }}
            >
              Next Day
              <ChevronRightIcon />
            </Link>
        ...
        <Dialog.Root>
          ...
          >
            <header>
              ...
              <Dialog.Description>
                URLs matching patterns in this list will not be publicly visible
                or indexed.
              </Dialog.Description>
            </header>
      ...
      <ScrollArea>
        <DataGrid
          ...
          sorting={true}
          getRowId={(row) =>
            `${row.hostname}-${row.url.href}-${row.firstVisit}`
          }
        />
      </ScrollArea>
    </main>
